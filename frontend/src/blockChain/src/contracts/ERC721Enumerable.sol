...
contract ERC721Enumerable is IERC721Enumerable, ERC721URIStorage {
  ...
  mapping(string => bool) private _tokenURIExist;
  ...
  function _mintSafeMain(address to, uint256 tokenId, string calldata URI) internal {
    require(_tokenURIExist[URI] == false, "ERROR: Token URI is already used");
    _mint(to, tokenId);
    _setTokenURI(tokenId, URI);
    _tokenURIExist[URI] = true;
  }
  ...
  function _ownedTokensIds(address owner) public view returns (uint256[] memory){
