...
contract ComposableBottomUp is ERC721c, ERC998ERC721BottomUp, ERC998ERC721BottomUpEnumerable {
...
*/
  ...
  function transferFromParent(address _fromContract, uint256 _fromTokenId, address _to, uint256 _tokenId, bytes _data) external {
    ...
    if (isContract(_to)) {
      bytes4 retval = ERC721TokenReceiverc(_to).onERC721Received(msg.sender, _fromContract, _tokenId, _data);
...
require(ERC721c(_toContract).ownerOf(_toTokenId) != address(0), "_toTokenId does not exist");
