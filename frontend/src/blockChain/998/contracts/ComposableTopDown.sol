...
import "./ERC721c.sol";
import "./ERC721Receiverc.sol";
import "./SafeMathc.sol";
...
interface ERC998ERC721TopDown {
  ...
  function ownerOfChild(address _childContract, uint256 _childTokenId) public view returns (bytes32 parentTokenOwner, uint256 parentTokenId);
  ...
  //function safeTransferChild(uint256 _fromTokenId, address _to, address _childContract, uint256 _childTokenId) internal;

  //function safeTransferChild(uint256 _fromTokenId, address _to, address _childContract, uint256 _childTokenId, bytes _data) internal;
...
contract ComposableTopDown is ERC721c, ERC998ERC721TopDown, ERC998ERC721TopDownEnumerable {
...
ERC721c(_childContract).safeTransferFrom(this, _to, _childTokenId);
...
rootOwnerAndTokenIdToApprovedAddress[rootOwner][tokenId] == msg.sender);
removeChild(tokenId, _childContract, _childTokenId);
ERC721c(_childContract).safeTransferFrom(this, _to, _childTokenId, _data);
emit TransferChild(tokenId, _to, _childContract, _childTokenId);
}
...
ERC721c(_childContract).transferFrom(this, _to, _childTokenId);
emit TransferChild(tokenId, _to, _childContract, _childTokenId);
}
...
function getChild(address _from, uint256 _tokenId, address _childContract, uint256 _childTokenId) internal {
receiveChild(_from, _tokenId, _childContract, _childTokenId);
require(_from == msg.sender ||
ERC721c(_childContract).isApprovedForAll(_from, msg.sender) ||
ERC721c(_childContract).getApproved(_childTokenId) == msg.sender);
ERC721c(_childContract).transferFrom(_from, this, _childTokenId);
...
require(ERC721c(msg.sender).ownerOf(_childTokenId) != address(0), "Child token not owned.");
...
function childExists(address _childContract, uint256 _childTokenId) public view returns (bool) {
uint256 tokenId = childTokenOwner[_childContract][_childTokenId];
return childTokenIndex[tokenId][_childContract][_childTokenId] != 0;
}

function totalChildContracts(uint256 _tokenId) public view returns (uint256) {
return childContracts[_tokenId].length;
}

function childContractByIndex(uint256 _tokenId, uint256 _index) public view returns (address childContract) {
...
function childTokenByIndex(uint256 _tokenId, address _childContract, uint256 _index) public view returns (uint256 childTokenId) {
